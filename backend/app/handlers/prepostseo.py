...
class PrePostSEOHandler(BaseHandler):

    @property
    def site_name(self):
        return Sites.PREPOSTSEO["name"]

    @property
    def site_url(self):
        return Sites.PREPOSTSEO["url"]
    ...
    async def submit(
        ...
        try:
            ...
            #
            await page.fill(
                Selectors.PREPOSTSEO["url_input"],
                str(request.url),
            )
            ...
            #
            await page.click(
                Selectors.PREPOSTSEO["next_button"]
            )
            ...
            # Start Pinging
            #
            print("About to Start Pinging")

            await page.get_by_text(
                "Start Pinging"
            ).click(force=True)

            print("✓ Start Pinging Clicked")
